...
const seed: Msg[] = [
  ...
  { id: "2", role: "user", text: "Can't sleep, exam stress" },
  {
    id: "3",
    role: "bot",
    text: "Oof that's rough! Let's try the 4-7-8 breathing technique...",
  },
];
    ...
    <div className="grid lg:grid-cols-[2fr_1fr] gap-6">
      <div className="rounded-xl border bg-background overflow-hidden flex flex-col">
        <div className="px-4 py-3 border-b bg-muted/40 text-sm font-medium">
          MITRA Chat 💜 - Your 24/7 Mental Health Bestie
        </div>
        <div className="flex-1 p-4 space-y-3 overflow-auto">
          {messages.map((m) => (
            <div
              key={m.id}
              className={`max-w-[80%] rounded-2xl px-3 py-2 ${m.role === "bot" ? "bg-gradient-to-r from-[#667eea]/20 to-[#764ba2]/20" : "bg-primary text-primary-foreground ml-auto"}`}
            >
              {m.text}
            </div>
          ))}
        ...
        <div className="rounded-xl border p-4">
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            <Button variant="outline" asChild>
              <a href="/app/crisis">Crisis Help</a>
            </Button>
            <Button
              variant="outline"
              onClick={() =>
                setMessages((m) => [
                  ...m,
                  {
                    id: Math.random().toString(36).slice(2),
                    role: "bot",
                    text: "Inhale 4, hold 7, exhale 8. Repeat 4 times.",
                  },
                ])
              }
            >
            ...
            <Button variant="outline" asChild>
              <a href="tel:9820466726">Call Counselor</a>
            </Button>
            <Button variant="outline" asChild>
              <a href="/app/student/mood">Mood Tracker</a>
            </Button>
          </div>
        </div>
        <div className="rounded-xl border p-4">
    ...
    <div className="flex items-center gap-1">
      {moods.map((m) => (
        <button
          key={m}
          onClick={() => onSelect(m)}
          className="size-8 grid place-items-center text-lg hover:scale-110 transition-transform"
        >
          {m}
        </button>
      ))}
...
function replyTo(text: string) {
  ...
  if (t.includes("sleep") || t.includes("insomnia"))
    return "Try putting your phone away 30 mins before bed and dim lights. We can do a body scan together.";
  if (t.includes("exam") || t.includes("stress"))
    return "Totally get it. Let's break tasks into tiny chunks and do 10 focused minutes. Ready?";
  if (t.includes("sad"))
    return "I'm here. Want to talk about what's making you feel this way?";
